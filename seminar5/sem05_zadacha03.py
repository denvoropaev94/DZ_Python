...
def drawBoard(board):
    # Функция выводит на экран игровое поле, клетки которого будут заполняться.
...
def inputPlayerLetter():
    ...
    # Возвращает список, в котором буква игрока - первый элемент, а буква компьютера - воторой.
    letter = ''
...
def whoGoesFirst():
    # Случайный выбор игрока, который ходит первый
...
def makeMove(board, letter, move):  # Сделать ход
    board[move] = letter
...
def isWinner(bo, le):
    ...
    return ((bo[7] == le and bo[8] == le and bo[9] == le) or
            (bo[6] == le and bo[5] == le and bo[4] == le) or
            (bo[6] == le and bo[5] == le and bo[4] == le) or
...
def isSpaceFree(board, move):
    # Возвращает True, если сделан ход в пустую клетку.
    return board[move] == ' '


def getPlayerMove(board):
    # Разрешение сделать игроку ход.
    move = ' '
...
def chooseRandomMoveFromList(board, moveList):
    ...
    # Возвращает значение None, если больше нет опустимых ходов.
    possibleMoves = []
    for i in moveList:
...
def isBoardFull(board):
    # Возвращет True, если клетка на игровом поле занята. В противном случае возвращает False
    for i in range(1, 10):
        if isSpaceFree(board, i):
            return False
    return True
